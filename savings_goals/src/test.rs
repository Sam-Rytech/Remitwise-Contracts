...
#[test]
fn test_get_all_goals_many_items() {
    let env = Env::default();
    let contract_id = env.register_contract(None, SavingsGoalContract);
    let client = SavingsGoalContractClient::new(&env, &contract_id);
    ...
    for i in 0..12 {
        ...
        created_goal_ids.push_back(goal_id);
    }

    // Test get_all_goals returns correct count
    let all_goals = client.get_all_goals(&owner);
    assert_eq!(all_goals.len(), 12, "Should return exactly 12 goals");

    // Verify all returned goals belong to the owner
    for i in 0..all_goals.len() {
        let goal = all_goals.get(i).unwrap();
        assert_eq!(goal.owner, owner, "All goals should belong to the owner");
    }

    // Verify all goal IDs are distinct and in expected range (1..=12)
    ...
    for i in 0..found_ids.len() {
        for j in (i + 1)..found_ids.len() {
            assert_ne!(
                ...
                "Goal IDs should be unique"
            );
        }
    }

    // Test isolation: create goals for different owner
    let other_owner = Address::generate(&env);
    client.create_goal(
        &other_owner,
        &String::from_str(&env, "Other Owner Goal"),
        &5000,
        &1735689600,
    );
